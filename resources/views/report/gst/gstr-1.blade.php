@extends('layouts.app')
@section('title', __('item.gstr-1-report'))
        ...
        <div class="page-wrapper">
            <div class="page-content">
                ...
                <div class="row">
                    <form class="row g-3 needs-validation" id="reportForm" action="{{ route('report.gstr-1.ajax') }}" enctype="multipart/form-data">
                        {{-- CSRF Protection --}}
                        @csrf
                        @method('POST')
                        ...
                        <div class="col-12 col-lg-12">
                            <div class="card">
                                <div class="card-header px-4 py-3">
                                    <h5 class="mb-0">{{ __('item.gstr-1-report') }}</h5>
                                </div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body p-4 row g-3">
                                        <div class="col-md-12 table-responsive">
                                            <table class="table table-bordered" id="gstr1Report">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        ...
                                                        <th>{{ __('app.date') }}</th>
                                                        <th>{{ __('sale.invoice_value') }}</th>
                                                        <th>{{ __('tax.tax_rate') }}</th>
                                                        <th>{{ __('tax.taxable_value') }}</th>
                                                        <th>{{ __('tax.cgst') }}</th>
                                                        <th>{{ __('tax.sgst') }}</th>
                                                        <th>{{ __('tax.igst') }}</th>
                                                        <th>{{ __('party.state_of_supply') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody></tbody>
                                            </table>
...
@section('js')
    ...
    <script src="{{ versionedAsset('custom/js/reports/gst/gstr-1.js') }}"></script>
